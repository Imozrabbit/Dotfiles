...
for (const zoom of [2, 3, 6]) {
    const root = {
        ...
        monthDate: new Date(2026, 8, 21), monthVisibleDate: new Date(2026, 8, 21),
    ...
    for (const [direction, year, month] of [[1, 2026, 9], [1, 2026, 10],
        ...
        navigate(direction)
        assert.equal(root.monthDate.getFullYear(), year)
        assert.equal(root.monthDate.getMonth(), month)
        assert.equal(root.monthDate.getDate(), 1)
    }
}
...
for (const year of [2012, 2024, 2026, 2027]) {
    for (const zoom of [2, 3, 4, 5, 6]) {
        const root = {
            ...
            pendingPositionOffset: 0, extendingBuffer: false
        }
        root.bufferWeeks = 0
        for (let cursor = root.bufferStart; cursor < root.bufferEnd; cursor = CalendarMath.addDays(cursor, 7))
            root.bufferWeeks++
...
for (const currentView of ["month", "week", "agenda"]) {
    ...
    const root = {
        currentView, weekDate: today, monthDate: today, agendaDate: today,
        clearViewSelection() {}, closeEventDetails() {}
    }
    let jumps = 0
    let resets = 0
    const item = {
        ...
        resetPosition(date) {
            if (currentView === "agenda")
                ...
                assert.equal(date.getTime(), today.getTime());
                ++jumps;
            }
        }
    }
    root.loadedView = item
...

{
    ...
    class FixedDate extends Date { constructor() { super(today.getTime()) } }
    const root = {
        currentView: "month", monthDate: today, monthVisibleDate: representative,
        clearViewSelection() {}, closeEventDetails() {}
    }
    root.loadedView = { resetPosition() {} }
    ...
    assert.equal(root.monthVisibleDate.getTime(), today.getTime(), "Today must reset Month visible date")
}
