...
assert.equal(CalendarMath.withCalendarVisibility, undefined)
assert.equal(typeof CalendarMath.holidayLabel, "function")
assert.equal(CalendarMath.holidayLabel([]), "")
assert.equal(CalendarMath.holidayLabel([{ title: "Holiday" }]), "Holiday")
assert.equal(CalendarMath.holidayLabel([{ title: "Holiday" }, { title: "Other" }]), "Holiday +1")
...

{
    ...
    const inheritedId = CalendarMath.filterEventsInRange([
        { uid: "hidden-constructor", calendarId: "constructor", startMs: 200, endMs: 300 }
    ], [{ id: "constructor", visible: false }], 200, 400)
    assert.deepEqual(inheritedId, [])
}
...

{
    ...
    assert.deepEqual(days[0].events.map(event => event.uid), ["ongoing-before"])
    ...
    assert.deepEqual(days[3].events.map(event => event.uid), ["multi-day"])
    assert.deepEqual(days[4].events.map(event => event.uid), ["multi-day"])
    assert.deepEqual(days[5].events, [])
    assert.equal(days.reduce((count, day) => count + day.events.length, 0), 6)
