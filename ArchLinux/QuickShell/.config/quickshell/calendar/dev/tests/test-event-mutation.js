...
assert.equal(EventMutation.parsePersonalStore, undefined)
assert.equal(EventMutation.serializePersonalStore, undefined)
...
assert.deepEqual(EventMutation.parseLocalStore(
    '{"version":1,"events":[]}', calendars, "personal"), {
    ok: true,
    rawEvents: []
})
assert.match(EventMutation.parseLocalStore("{}", calendars, "personal").message,
    /Local calendar schema is unsupported/)

{
    const localCalendars = calendars.concat([{
        id: "work", name: "Work", color: "#aa8866", writable: true, visible: true
    }])
    const workEvent = Object.assign({}, storedEvent, {
        uid: "work-event", calendarId: "work"
    })
    const parsed = EventMutation.parseLocalStore(
        JSON.stringify({ version: 1, events: [workEvent] }), localCalendars, "work")
    assert.deepEqual(parsed.rawEvents, [workEvent])
    assert.equal(EventMutation.parseLocalStore(
        JSON.stringify({ version: 1, events: [storedEvent] }), localCalendars, "work").ok, false)
    assert.equal(EventMutation.serializeLocalStore(
        [workEvent], localCalendars, "work").ok, true)
}

{
    const imported = {
        version: 1,
        events: [{
            uid: "edt_unistra:imported-event",
    ...
    const result = EventMutation.parseImportedStore(JSON.stringify(imported), "edt_unistra")
    assert.deepEqual(result.rawEvents, imported.events)
}

{
    const imported = {
        version: 1,
        events: [Object.assign({}, storedEvent, {
            uid: "edt_unistra:imported-reminder",
            calendarId: "edt_unistra"
        })]
    }
    const result = EventMutation.parseImportedStore(JSON.stringify(imported), "edt_unistra")
    assert.equal(result.ok, true)
    assert.deepEqual(result.rawEvents[0].reminders, [{ minutesBefore: 15 }])
    for (const reminders of [[{ minutesBefore: -1 }], [{ minutesBefore: 5 }, { minutesBefore: 5 }], [null]]) {
        imported.events[0].reminders = reminders
        assert.equal(EventMutation.parseImportedStore(JSON.stringify(imported), "edt_unistra").ok, false)
    }
}

{
    ...
    const result = EventMutation.parseSyncedStore(JSON.stringify(synced), calendars, "personal")
    assert.equal(result.ok, true)
    assert.equal(result.rawEvents[0].sourceUid, "stored")
    assert.equal(result.rawEvents[0].revision, "abc123")
}

{
    const holiday = {
        uid: "holidays_fr:2026-07-14",
        calendarId: "holidays_fr",
        title: "14 juillet",
        description: "",
        location: "",
        start: "2026-07-14",
    ...
    const result = EventMutation.parseImportedStore(
        JSON.stringify({ version: 1, events: [holiday] }), "holidays_fr")
    assert.deepEqual(result.rawEvents, [holiday])

    const normalized = EventMutation.normalizeHoliday(holiday)
    assert.equal(normalized.title, "14 juillet")
    assert.equal(normalized.allDay, true)
    assert.equal(normalized.endMs - normalized.startMs, 24 * 60 * 60 * 1000)

    const withReminder = Object.assign({}, holiday, { reminders: [{ minutesBefore: 15 }] })
    const parsedWithReminder = EventMutation.parseImportedStore(
        JSON.stringify({ version: 1, events: [withReminder] }), "holidays_fr")
    assert.equal(parsedWithReminder.ok, true)
    const normalizedWithReminder = EventMutation.normalizeHoliday(parsedWithReminder.rawEvents[0])
    assert.notEqual(normalizedWithReminder, null)
    assert.equal(normalizedWithReminder.title, "14 juillet")
    assert.deepEqual(normalizedWithReminder.reminders, [{ minutesBefore: 15 }])

    assert.equal(EventMutation.parseImportedStore(
        JSON.stringify({ version: 1, events: [holiday] }), "edt_unistra").ok, false)
}
...
for (const document of [
    ...
    { version: 1, events: [{
        uid: "edt_unistra:wrong-calendar",
        calendarId: "personal",
        title: "Bad calendar",
        description: "",
        location: "",
    ...
    const result = EventMutation.parseImportedStore(JSON.stringify(document), "edt_unistra")
...

{
    ...
    const result = EventMutation.parseImportedStore(
        JSON.stringify({ version: 1, events: [event, event] }), "edt_unistra")
    assert.equal(result.ok, false)
    assert.equal(result.field, "storage")
}

{
    const serialized = EventMutation.serializeLocalStore([storedEvent], calendars, "personal")
    assert.equal(serialized.ok, true)
    assert.equal(serialized.text.endsWith("\n"), true)
    assert.deepEqual(EventMutation.parseLocalStore(
        serialized.text, calendars, "personal").rawEvents, [storedEvent])
}
...
for (const text of [
    ...
    const result = EventMutation.parseLocalStore(text, calendars, "personal")
    assert.equal(result.ok, false)
    assert.equal(result.field, "storage")
}

{
    const result = EventMutation.parseLocalStore(
        JSON.stringify({ version: 1, events: [storedEvent] }),
        calendars, "personal", [storedEvent.uid])
    assert.equal(result.ok, false)
    assert.equal(result.field, "storage")
}
