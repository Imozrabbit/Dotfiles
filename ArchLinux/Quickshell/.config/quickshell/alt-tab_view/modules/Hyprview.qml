    ...
    function toggleExpose() {
        ...
        if (root.isActive) {
            if (root.layoutAlgorithm === 'random') {
                var layouts = ['smartgrid', 'justified', 'bands', 'masonry', 'hero', 'spiral', 'satellite', 'staggered', 'columnar', 'vortex',].filter(l => l !== root.lastLayoutAlgorithm);
            ...
            searchBox.reset();
            Hyprland.refreshToplevels();
            Qt.callLater(root.refreshThumbs);
        } else {
            ...
            Column {
                ...
                Item {
                    ...
                    ScriptModel {
                        ...
                        values: {
                            // Bailout on wrong screen size & inactivity
                            if (!root.isActive)
                                return [];
                            if (areaW <= 0 || areaH <= 0)
                                return [];
                    ...
                    Repeater {
                        ...
                        delegate: WindowThumbnail {
                            ...
                            targetX: modelData.x
                            targetY: modelData.y
                            targetZ: (visible && (exposeArea.currentIndex === index))
                            ? 1000
                            : (modelData ? (modelData.zIndex || 0) : 0)
                            targetRotation: modelData ? (modelData.rotation || 0) : 0
