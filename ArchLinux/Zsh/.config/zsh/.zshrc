...
# Setup arduino zsh auto completion
fpath=($HOME/.config/zsh/completion_zsh $fpath)
...
mkdir -p /tmp/zsh-vi
ZVM_TMPDIR=/tmp/zsh-vi

# Better vim mode for zsh
source $HOME/.config/zsh/better-vi-mode/zsh-vi-mode.plugin.zsh

# Override the open command for URLs
ZVM_OPEN_URL_CMD='zen-browser'
...
#---------------                          SSH                       ---------------#
#----------------------------------------------------------------------------------#
# Fastfetch on interactive SSH login
[[ -n "$SSH_TTY" ]] && fastfetch
